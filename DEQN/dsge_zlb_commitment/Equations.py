...
import PolicyState
import State
from Parameters import beta, gamma, epsilon, theta, omega, eps_CC
...
def equations_main(state,policy_state):
  ...
  #########################
    ...
    loss_dict['eq_1'] = ((c_t + g_t)**omega * (Delta_t/A_t)**(1+omega))**(-1) * ( #normalizing factor
        ...
        (varphi_t - varphi_old * ((1 + i_nom_old) * pi_aux_t**((epsilon - 1)/epsilon))/(pi_aux_t)) * gamma * c_t**(-gamma - 1)
    )
...
# #########################
#    #normalized equation
    loss_dict["eq_7"] = (1 + i_nom_t)**(-1) * (
        lambda_t / (beta * E_t(lambda sold,psold,snew,psnew: Definitions.pi_aux_y(snew,psnew) * Definitions.lambda_y(snew,psnew) / (Definitions.pi_aux_y(snew,psnew)**((epsilon+1)/epsilon)))) - (1 + i_nom_t))

#########################
   #normalized equation
    loss_dict['eq_8'] = (  #normalizing factor
        complementarity_function(chi_t,i_nom_t,eps_CC))

#########################
   #normalized equation
    loss_dict['eq_9'] = (p_star_aux_t)**(-1) * (  #normalizing factor
