...
import State
import Parameters
from Parameters import sigma_a_x, sigma_xi_x, rho_a_x, rho_xi_x, sigma_xi_x, p_12, p_21, rng, n_quad_pts, sim_mult, global_default_dtype, policy, sigma_g_x, rho_g_x
...
def policy_step(prev_state, policy_state):
    ...
    policy_step = State.update(policy_step, "vartheta_old_x", Definitions.vartheta_y(prev_state,policy_state))
    policy_step = State.update(policy_step, "rho_old_x", Definitions.rho_y(prev_state,policy_state))
    policy_step = State.update(policy_step, "c_old_x", Definitions.cons_y(prev_state,policy_state))
    policy_step = State.update(policy_step, "i_nom_old_x", Definitions.i_nom_y(prev_state,policy_state))
    policy_step = State.update(policy_step, "varphi_old_x", Definitions.varphi_y(prev_state,policy_state))
    return policy_step
