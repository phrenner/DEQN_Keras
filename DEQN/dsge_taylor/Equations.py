...
#########################
   ...
   #    normalized equation
   # loss_dict['eq_8'] = (
   #    (((1/beta*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar)))) - (i_nom_t))
      # (1 + ((1/beta*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar)))) - (1 + i_nom_t))/(1 + i_nom_t)
   
   # regime_t = State.regime_x(state)
   # target_t = (1 - regime_t) * 0.005320537256103819 / 400 + regime_t * 2.7053556952276736 / 400 + 1.
   # loss_dict['eq_8'] = (
   #    (( (target_t*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar)))) - (i_nom_t))
      # (1 + ((target_t*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar)))) - (1 + i_nom_t))/(1 + i_nom_t)
