...
import sys
from Parameters import gamma, omega, M_cal, beta, epsilon, theta, rho_i_x, g_bar, psi, pi_bar, r_nat_nt, r_nat_ss, global_default_dtype, p_12, p_21
...
def i_nom_y(state, policy_state):
    return (((1/beta*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar))))
    # return tf.nn.relu(((1/beta*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar))))

    # regime_t = State.regime_x(state)
    # target_t = (1 - regime_t) * r_nat_nt + regime_t * r_nat_ss + 1.
    # return (( (target_t*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar))))
    # return tf.nn.relu(((target_t*(1 + pi_bar) - 1 + psi * (Definitions.pi_tot_y(state,policy_state) - pi_bar))))
...
def pi_aux_y(state, policy_state):
    ...
    p_star_aux_t = Definitions.p_star_aux_y(state,policy_state)    
    return (-((-1 + p_star_aux_t**((epsilon - 1)/epsilon) - p_star_aux_t**((epsilon - 1)/epsilon) * theta)/theta))**(epsilon/(-1 + epsilon))
