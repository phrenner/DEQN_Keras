...
sigma_a_x = constants['sigma_a_x']
g_bar = constants['g_bar']
gamma = constants['gamma']
beta = constants['beta']
p_12 = constants['p_12']
p_21 = constants['p_21']
omega = constants['omega']
M_cal = constants['M_cal']

def cons_flex_eq(c_t,A_t,tau_t,g_t):
    return ((c_t + g_t) / A_t)**omega - A_t * c_t**(-gamma) / ( M_cal * (1+tau_t) )
def cons_flex_init(A_t,tau_t):
    return (A_t**(1+omega) / ( M_cal * (1+tau_t) ))**(1/(omega+gamma))

tau = 0.0 # bad state
c_nat_ss = scipy.optimize.newton(lambda c: cons_flex_eq(c,np.exp(-0.5 * sigma_a_x**2),tau,g_bar), cons_flex_init(1,tau))
tau = -1/epsilon # good state
c_nat_nt = scipy.optimize.newton(lambda c: cons_flex_eq(c,np.exp(-0.5 * sigma_a_x**2),tau,g_bar), cons_flex_init(1,tau))
r_nat_nt = 1/beta * c_nat_ss**(gamma) / (p_12 * c_nat_nt**(gamma) + (1 - p_12) * c_nat_ss**(gamma)) - 1
r_nat_ss = 1/beta * c_nat_nt**(gamma) / ((p_21) * c_nat_ss**(gamma) + (1-p_21) * c_nat_nt**(gamma)) - 1

constants['r_nat_nt'] = r_nat_nt
constants['r_nat_ss'] = r_nat_ss
...
### total state space
states_main = end_state + ex_state
...
# policies_main.append({'name':'pi_aux_y'})
...
# definitions.append({'name':'i_nom_y', 'bounds': {'lower': 0., 'penalty_lower': 10000.}})
