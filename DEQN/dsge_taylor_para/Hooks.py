...
from Parameters import rho_xi_x, sigma_g_x, rho_g_x, ENABLE_TB, pi_bar, beta, p_21_l, p_21_u
import PolicyState
import State
import Definitions
...
def post_init():
    ...
    normal_noise = tf.random.normal(
        ...
        stddev=tf.constant([[
            0.002, # disp
            0.008, # i_nom
            sigma_a_x**2/(1-rho_a_x**2), # a_t
            sigma_xi_x**2/(1-rho_xi_x**2), # xi_t
            sigma_g_x**2/(1-rho_g_x**2) # g_t
            ]],dtype=global_default_dtype),
        dtype=global_default_dtype)


    p_21_vec = tf.random.uniform([nB],p_21_l,p_21_u,dtype=global_default_dtype)
    Parameters.starting_state.assign(
        State.update(Parameters.starting_state, "p_21_x", p_21_vec))
    ...
    if Parameters.ckpt.current_episode < 2:    
        ...
        Parameters.starting_state.assign(State.update(Parameters.starting_state,'i_old_x', 
                                                    tf.constant(i_nom_SS,shape=(Parameters.starting_state.shape[0],),dtype=global_default_dtype) + normal_noise[:,1]))
        ...
        Parameters.starting_state.assign(State.update(Parameters.starting_state,'log_g_x', 
                                                    tf.constant(0.,shape=(Parameters.starting_state.shape[0],),dtype=global_default_dtype) + normal_noise[:,4]))
