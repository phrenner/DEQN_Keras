...
import State
import Parameters
from Parameters import sigma_a_x, sigma_xi_x, rho_a_x, rho_xi_x, sigma_xi_x, p_12, p_21, rng, n_quad_pts, sim_mult, global_default_dtype, policy, sigma_g_x, rho_g_x, p_21_l, p_21_u
    ...
    for indx_ax in range(n_quad_pts):
        for indx_xi in range(n_quad_pts):
            for indx_g in range(n_quad_pts):
                ...
                indx_lst.append([indxm])
                tmp_shock_prob_SS[counter] = probs_gauss_hermite[indx_ax] * probs_gauss_hermite[indx_xi] * probs_gauss_hermite[indx_g]
...
part_shock_prob_SS = tf.constant(tmp_shock_prob_SS,dtype=global_default_dtype)
...
def total_step_spec_probs(prev_state, policy_state, mode=None):
    ...
    p_21 = tf.expand_dims(State.p_21_x(prev_state),0)
    tmp_trans_prob_regime_SS = tf.concat([p_21,1 - p_21],0)

    shock_prob_SS = tf.transpose(tf.gather_nd(tmp_trans_prob_regime_SS,indx_lst,0)) * tf.expand_dims(part_shock_prob_SS,0)
    shock_prob_SS  = shock_prob_SS/tf.math.reduce_sum(shock_prob_SS,-1,keepdims=True)    

    regime_in = tf.repeat(State.regime_x(prev_state),n_int_samples,axis=0)
    shock_p_vec_1 = tf.tile(shock_prob_NT,[prev_state.shape[0]])
    shock_p_vec_2 = tf.reshape(shock_prob_SS,[n_int_samples*nB])
...
def shock_step_random(prev_state):
    shock_step = tf.zeros_like(prev_state)
    random_normals = tf.cast(rng.normal([prev_state.shape[0],3]),dtype=global_default_dtype)
    ...
    shock_prob_SS = tf.transpose(tmp_trans_prob_regime_SS)

    current_trans_prob = tf.expand_dims(1 - State.regime_x(prev_state),-1) * tf.expand_dims(trans_prob_regime_NT,0) + tf.expand_dims(State.regime_x(prev_state),-1) * shock_prob_SS
...
def policy_step(prev_state, policy_state):
    ...
    policy_step = State.update(policy_step, "p_21_x", State.p_21_x(prev_state))
...
def total_step_no_shock(prev_state, policy_state,n_batches,n_grid_pts):
    ar = AR_step(prev_state)
...
def total_step_shock(prev_state, policy_state,n_batches,n_grid_pts):
    ar = AR_step(prev_state)
    
    shock = ir_shock(prev_state,n_batches,n_grid_pts)
...
def ir_shock(prev_state,n_batches,n_grid_pts):
    ...
    shock_step = State.update(shock_step, "regime_x", tf.repeat(regimes,n_grid_pts,0))  #maintain regime
    ...
    shock_step = State.update(shock_step,'log_a_x',tf.repeat(shock_vec_a_x,n_grid_pts,0))
    ...
    shock_step = State.update(shock_step,'log_xi_x',tf.repeat(shock_vec_xi_x,n_grid_pts,0))
    ...
    shock_step = State.update(shock_step,'log_g_x',tf.repeat(shock_vec_g_x,n_grid_pts,0))
