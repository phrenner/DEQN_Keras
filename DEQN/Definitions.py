...
for d in definitions:
    if (d in definition_bounds_hard['lower']) or (d in definition_bounds_hard['upper']):
        setattr(sys.modules[__name__],d, 
                (lambda de: lambda s, ps: tf.clip_by_value(getattr(Model_Definitions,de)(s, ps), definition_bounds_hard['lower'].get(de, -np.inf), definition_bounds_hard['upper'].get(de,np.inf)))(d)
                )
    else:
