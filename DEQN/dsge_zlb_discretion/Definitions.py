...
def H_fun_y(state,policy_state):
    return Definitions.cons_y(state,policy_state)**(-gamma) * (Definitions.pi_aux_y(state,policy_state))**((epsilon-1)/epsilon) / Definitions.pi_aux_y(state,policy_state)
...
def chi_y(state, policy_state):
    ...
    return - beta * Definitions.varphi_y(state,policy_state) * (
        E_t(lambda sold,psold,snew,psnew: Definitions.H_fun_y(snew,psnew)))
...
def dH_dDelta(state,policy_state):
   Delta_old = State.__dict__['disp_old_x'](state)
   with tf.GradientTape() as tape:
       tape.watch(Delta_old)
       state_new = tf.identity(state)
       state_new = State.update(state_new,'disp_old_x', Delta_old)
       policy_new = policy(state_new)
       out = Definitions.H_fun_y(state_new,policy_new)

   dH_dD = tape.gradient(out,Delta_old)
   return dH_dD
