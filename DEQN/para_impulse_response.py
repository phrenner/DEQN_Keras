...
import os
import sys
from Parameters import global_default_dtype, p_21_l, p_21_u
...
@tf.function
def do_step_no_shock(current_state,n_batches,n_grid_pts):
    return Dynamics.total_step_no_shock(current_state, Parameters.policy(current_state),n_batches,n_grid_pts)

@tf.function
def do_step_shock(current_state,n_batches,n_grid_pts):
    return Dynamics.total_step_shock(current_state, Parameters.policy(current_state),n_batches,n_grid_pts)


def run_episode(state_episode,shock_indx, para_grid):
    """ Runs an episode starting from the begging of the state_episode. Results are saved into state_episode."""
    n_grid_pts = para_grid.shape[0]
    ...
    current_state = tf.repeat(State.update(current_state,"regime_x",regimes),n_grid_pts,0)
    current_state = State.update(current_state,"p_21_x",tf.tile(para_grid,[n_batches]))

    state_episode = tf.repeat(state_episode,n_grid_pts,1)
    
    # run optimization
    for i in range(1, state_episode.shape[0]):
        if i == shock_indx:
            current_state = do_step_shock(current_state,n_batches,n_grid_pts)
        else:
            current_state = do_step_no_shock(current_state,n_batches,n_grid_pts)
...
n_grid_pts = 11
n_batches = (2*12 + 2)
para_grid = tf.cast(tf.linspace(p_21_l, p_21_u, n_grid_pts),dtype = global_default_dtype)
para_grid = tf.constant([0.01666667,1./50.,1./40., 1./30., 1./20.,1./10. , 0.115     , 0.21333334, 0.3116667 , 0.41000003,
       0.5083333 , 0.6066667 , 0.705     , 0.8033333 , 0.90166664,
       1.        ], dtype=global_default_dtype)
n_grid_pts = para_grid.shape[0]
...
state_episode = run_episode(state_episode,no_steps,para_grid)[no_steps-presteps:,:,:]

flat_state_episode = tf.reshape(state_episode,[(no_steps_decay + presteps)*n_batches*n_grid_pts,state_episode.shape[-1]])
flat_policy_episode = Parameters.policy(flat_state_episode)
        
policy_episode = tf.reshape(
    flat_policy_episode,
        [no_steps_decay + presteps,n_batches*n_grid_pts,len(Parameters.policy_states)])


def_lst = ["cons_y","disp_y","i_nom_y","h_work_y","pi_tot_y","pi_aux_y","p_star_y","p_star_aux_y","r_real_y","tau_x","a_x","xi_x", "g_x", "y_tot_y", "out_gap_y", "i_flex_y", "cons_flex_y", "regime_x", "p_21_x"]
...
n_time_steps = state_episode.shape[0]
policy_dim = policy_episode.shape[-1]
state_dim = state_episode.shape[-1]
...
for indxb in range(n_batches):
    ...
    out_dict = {**out_dict , **{flat_shock_lst[indxb]: {d : tf.reshape(getattr(Definitions,d)(
        tf.reshape(state_episode[:,indxb*n_grid_pts : (indxb + 1) * n_grid_pts,:],[n_time_steps*n_grid_pts,state_dim]), 
        tf.reshape(policy_episode[:,indxb*n_grid_pts : (indxb + 1) * n_grid_pts,:],[n_time_steps*n_grid_pts,policy_dim])
    ),[n_time_steps,n_grid_pts]) for d in def_lst}}}
